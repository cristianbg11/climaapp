...
?>
    ...
    ?>
    <h2>Graficos por etapa de cultivos</h2>
    <ul>
   <?php // Vergraf($id,$idcultivo,$idfinca)?>
    <?php foreach ($fincascultiv as $culfinca):?>

        <li><i class="fa fa-calendar"></i><a href="<?=Url::to(['predicciong/vergraf','id'=>$model->id,'idcultivo'=>$culfinca->id_cultivo,'idfinca'=>$culfinca->id_finca])?>"> <?php echo $culfinca->cultivo->Cultivo.' '.$culfinca->finca->Nombre; ?> </a></li>
        <?php endforeach;?>
    </ul>
</div>
